...
export const ChangeMeetForm = ({ url, meet }) => {
  ...
  const onSubmit = (data) => {
    ...
    axios.put(url + `meets/`, meet, getConfig()).then(() => navigate(`/`));
  };
        ...
        <div className={style.communicationGames}>
          <div className={style.communication}>
            ...
            <ul className={style.gamesList}>
              <label className={style.inputGreenLabel}>
                {t("meetsPage.form.inputMembers.name")}
              </label>
              {playerList?.length == 0
                ? t("meetsPage.form.inputMembers.notMembers")
                : null}
              {playerList?.length != 0 &&
                playerList?.length &&
                playerList.map((player) => (
                  <li key={player.id} className={style.gameItem}>
                    <button
                      ...
                      onClick={() => minusPlayerButtonHandler(player.id)}
                    >
                      <MinusButton />
                    </button>
                    <NavLink to={`/user/${player.id}`}>
                      @{player.userName}
                    </NavLink>
                  </li>
                ))}
                ...
                gameList.map((game) => (
                  <li key={gameList.indexOf(game)} className={style.gameItem}>
                    <button
                      type="button"
                      className={style.minusButton}
                      onClick={() => minusGameButtonHandler(game)}
                    >
                      <MinusButton />
                    </button>
                    {game}
